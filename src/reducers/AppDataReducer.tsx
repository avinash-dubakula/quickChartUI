...
const AppDataReducer = (
    ...
    if (action.type != null) {
        switch (action.type) {
            ...
            case 'SetActiveChat':
                if (
                    action.currentChatUserName != null &&
                    oldState.activeChatUserName != action.currentChatUserName
                ) {
                    //Start the messageId Flush
                    var newState = { ...oldState }
                    if (newState.chats != null) {
                        var chatTobeUpdated = newState.chats.chatData.find(
                            (chat) =>
                                chat.chat.friendUserName ==
                                oldState.activeChatUserName
                        )
                        if (chatTobeUpdated != undefined) {
                            chatTobeUpdated.newMessageIds = []
                            return {
                                ...newState,
                                activeChatUserName: action.currentChatUserName,
                            }
                        }
                    }
            ...
            case 'SentMessage':
                if (action.messageResponse != null) {
                    var resultState = SentMessageAction(
            ...
            case 'RecievedMessage':
                if (action.messageResponse != null && action.updateStatus) {
                    var resultState = RecieveMessageAction(
                        action.messageResponse,
                        oldState,
                        action.updateStatus
                    )
            ...
            case 'RecievedMessageUpdate':
                if (action.messageResponse != null) {
                    var resultState = RecieveMessageUpdateAction(
                        action.messageResponse,
                        oldState
                    )
                    return resultState
            ...
            case 'RecievedMessagesUpdate':
                if (action.messagesUpdate != null) {
                    var resultState = MessagesUpdateAction(
            ...
            case 'SentSeenAck':
                if (action.currentChatUserName != null) {
                    var newState = { ...oldState }
                    var temp = newState.chats?.chatData.find(
                        (item) =>
                            item.chat.friendUserName ==
                            action.currentChatUserName
                    )
            ...
            case 'DeliveryStatusSent':
                ...
                return resultState
            case 'REMOVEACTIVECHAT':
                return { ...oldState, activeChatUserName: null }
            case 'LOGOUT':
