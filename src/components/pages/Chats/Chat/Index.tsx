...
import {
    ...
    IMessagesSeenModel,
    MessageStatus,
} from '../../../../types/AppData/Message/Types'
...
} from '../../../../types/AppData/Context/Types'
...
const Chat: React.FC = () => {
    ...
    const sendSeenAck = async (latestMId: number) => {
        if (
            ...
            AuthData.userAuthenticationData.userName !=
                AppData.activeChatUserName
        ) {
            console.warn(
                `In UseEffect ${latestMId} ${chat.seenTillMessageId} ${seenAckId}`
            )
            var latestMessageId = chat.chat.lastMessageId
            if (
                (chat.seenTillMessageId == null ||
                    chat.seenTillMessageId < latestMessageId) &&
                seenAckId != latestMessageId
            ) {
                var seenModel: IMessagesSeenModel = {
                    FriendUserName: AppData.activeChatUserName,
                    SeenTillMessageId: latestMessageId,
                }
                var res = await PutSeenAck<Boolean>(
                ...
                if (res === true) {
                    ...
                    appDispatch({
                        ...InitialAppDataActionType,
                        type: 'SentSeenAck',
                        currentChatUserName: AppData.activeChatUserName,
                        seenAckId: latestMessageId,
                    })
                }
            }
    ...
    if (AppData.chats != null && AppData.activeChatUserName != null) {
        ...
        if (chat != null) {
            ...
            if (signalRData.activeUserNames != null) {
                isUserActive = signalRData.activeUserNames.includes(
                    userChat.friendUserName
                )
            }
        }
    }
    ...
    useEffect(() => {
        ...
        chatContainerRef.current?.addEventListener('scroll', (e) =>
            onScroll(chatContainerRef, setShowScrollIcon)
        )

        // Cleanup function
        return () => {
            document.removeEventListener('mousedown', onOutsideClick)
            chatContainerRef.current?.removeEventListener('scroll', (e) =>
                onScroll(chatContainerRef, setShowScrollIcon)
            )
        }
        ...
        <div className='chat-container'>
            ...
            <div className='chat' ref={chatContainerRef}>
                <MessageList
                    ...
                    newMessageStartId={newMessageStart}
                />
            </div>
            <div
                className='message-sending-container'
