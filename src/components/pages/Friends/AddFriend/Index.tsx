...
const AddFriend: React.FC = () => {
    ...
    const {Data:friendShipData,dispatch:friendShipDispatc}=useContext(FriendshipContext);
    ...
    const sentUserNames=(friendShipData.friendRequests?.friendRequestsSent??[]).map((r,index)=>(r.senderUserName))
    ...
    const fetchUsers = async (searchText: string) => {
        ...
        if (token) {
            ...
            try {
                const searchResult = await FetchUsers<IUserData>(token, searchText, { signal: controller.signal });
                if (searchResult) {
        ...
        <div className='add-friend-container'>            
            ...
            <div className='add-friend-body'>
                {users.map((user, index) => (
                    <div className='d-flex w-75 m-2 p-2 bg-white add-friend-card' key={`add-friend-${user.userName}`}>
                      <div style={{width:'15%'}} className='d-flex flex-column justify-content-center align-items-center'>
                      ...
                      {user.profilePhotoUrl?.length > 0 ? (
                        <div className="profile-container text-bg-">
                          ...
                          {activeUserNames.includes(user.fullName) && <div className="online-status"></div>}
                        </div>
                      ) : (
                        <div className="profile-container">
                          <div className="profile-initials">{getInitials(user.fullName??"Dummy rao")}</div>
                          {activeUserNames.includes(user.fullName) && <div className="online-status"></div>}
                        </div>
                      )}
                    </div>
                      ...
                      <div className=''style={{width:'85%'}} >
                        <div className="w-100 d-flex justify-content-between align-items-center pr-3">
                          <h3 className='m-0 p-0 friend-name-display'>{user.fullName}</h3>
                          <div className="">
                            {user.userName}  
                          </div>
                        </div>
                        <div className="w-100 d-flex justify-content-between align-items-center pr-3">
                          ...
                          <div className="">
                            {
                              sentUserNames.includes(user.userName)?<button className='p-2 bg-info text-white border-0 rounded disabled'>Friend Request Sent </button>:<button className='p-2 bg-success text-white border-0 rounded' onClick={()=>handleAddFriend(user.userName)}> + Add friend</button>
                            }
                          </div>
                        </div>
                      </div>
